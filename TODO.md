...
- [x] Créer la classe `RecapModel`.
  ...
  - [x] **Auto-load** : Ajouter l'appel à la fonction au chargement (`window.onload`).
        - [x] (pas directement inserer dans la base de donnee, juste histoire de voir(visualiser) l'action va se passer      prochainement)
    - [c] bouton validation qui dispatch vraiment les dons 
        - [x] (insertion directe dans la base de ce qu'on venait de voir dans la simulation)
